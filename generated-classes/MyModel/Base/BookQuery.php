...
namespace MyModel\Base;

use \Exception;
use \PDO;
use MyModel\Book as ChildBook;
use MyModel\BookQuery as ChildBookQuery;
use MyModel\Map\BookTableMap;
 ...
 *
 * @method     \MyModel\PublisherQuery|\MyModel\AuthorQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class BookQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \MyModel\Base\BookQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\MyModel\\Book', $modelAlias = null)
    {
    ...
    /**
     * Filter the query by a related \MyModel\Publisher object
     *
     * @param \MyModel\Publisher|ObjectCollection $publisher The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
    ...
    {
        if ($publisher instanceof \MyModel\Publisher) {
            return $this
            ...
            throw new PropelException('filterByPublisher() only accepts arguments of type \MyModel\Publisher or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \MyModel\PublisherQuery A secondary query class using the current class as primary query
     */
    public function usePublisherQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinPublisher($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Publisher', '\MyModel\PublisherQuery');
    }

    /**
     * Filter the query by a related \MyModel\Author object
     *
     * @param \MyModel\Author|ObjectCollection $author The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
    ...
    {
        if ($author instanceof \MyModel\Author) {
            return $this
            ...
            throw new PropelException('filterByAuthor() only accepts arguments of type \MyModel\Author or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \MyModel\AuthorQuery A secondary query class using the current class as primary query
     */
    public function useAuthorQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinAuthor($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Author', '\MyModel\AuthorQuery');
    }
